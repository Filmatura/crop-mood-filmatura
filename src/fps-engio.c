...
static struct menu_entry fps_menu[] = {
    #ifdef FEATURE_FPS_OVERRIDE
    {
        ...
        /* Under slim menus, SET only opens .children when EM_INLINE_ADJUST
         * is set (see menu_entry_select, mode 3); otherwise SET just calls
         * .select like a plain toggle and the submenu is unreachable.
         * L/R dial still toggles fps_override via .select as before. */
